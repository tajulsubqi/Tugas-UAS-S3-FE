...
export function useMahasiswa() {
  ...
  const fetchMahasiswa = useCallback(async (params: MahasiswaQueryParams = {}) => {
    ...
    setError(null);
    try {
      const response = await api.get<PaginatedResponse<Mahasiswa>>("/api/mahasiswa", { params });
  ...
  const createMahasiswa = async (data: MahasiswaCreate) => {
    ...
    try {
      ...
      const errData = (err as { response?: { data?: { message?: string; errors?: { field: string; message: string }[] } } })?.response?.data;
      return {
        success: false,
